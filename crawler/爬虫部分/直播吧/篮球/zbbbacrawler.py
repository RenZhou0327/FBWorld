...
def get_all_linksforsports(content, page):
    ...
    urls = re.findall(r"//news.zhibo8.cc/nba/.*?.htm",content,re.I)#https://sports.sina.com.cn/china/j/2019-12-13/doc-iihnzahi7174738.shtml
    #url1=re.findall(r"http://sports.sina.com.cn/csl/.+.shtml",content,re.I)
    for u in urls:
        if(u.startswith('https')==False):
            u="https:"+u
        sportslinks.append(u)
    return sportslinks

def add_page_to_folder(page, content):  # 将网页存到文件夹里，将网址和对应的文件名写入index.txt中^
    index_filename = 'zzbglbaindex1.txt'  # index.txt中每行是'网址 对应的文件名'
    folder = 'zbbglba1'  # 存放网页的文件夹
...
if __name__ == '__main__':
    ...
    start = time.clock()
    q=Queue()
    q.put('https://news.zhibo8.cc/nba/')
    threads=[]
    for i in range(10):
