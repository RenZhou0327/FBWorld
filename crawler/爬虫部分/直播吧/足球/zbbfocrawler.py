...
def get_all_linksforsports(content, page):
    ...
    urls = re.findall(r"//news.zhibo8.cc/zuqiu/.*?.htm",content,re.I)#https://sports.sina.com.cn/china/j/2019-12-13/doc-iihnzahi7174738.shtml
...
def add_page_to_folder(page, content):  # 将网页存到文件夹里，将网址和对应的文件名写入index.txt中^
    index_filename = 'zbbfoindex.txt'  # index.txt中每行是'网址 对应的文件名'
    folder = 'zbbfo'  # 存放网页的文件夹
...
if __name__ == '__main__':
    ...
    start = time.clock()
    q=Queue()
    q.put('https://news.zhibo8.cc/zuqiu/')
    threads=[]
    for i in range(10):
